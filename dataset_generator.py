...
def get_list_of_images(root_dir, N=1):
    ...
    '''
    img_list = []
    image_search = lambda ext : glob.glob(root_dir + ext, recursive=True)
    for ext in ['/**/*.jpg', '/**/*.jpeg', '/**/*.png']: img_list.extend(image_search(ext))
...
def create_image_anno(objects, distractor_objects, img_file, anno_file, bg_file,  w=WIDTH, h=HEIGHT, scale_augment=False, rotation_augment=False, blending_list=['none'], dontocclude=False):
    ...
    while True:
        ...
        for idx, obj in enumerate(all_objects):
           ...
           filename.text = str(os.path.basename(img_file))
...
def gen_syn_data(img_files, labels, img_dir, anno_dir, scale_augment, rotation_augment, dontocclude, add_distractors):
    ...
    background_files = []
    image_search = lambda ext : glob.glob(background_dir + ext, recursive=True)
    for ext in ['/**/*.jpg', '/**/*.jpeg', '/**/*.png']: background_files.extend(image_search(ext))
